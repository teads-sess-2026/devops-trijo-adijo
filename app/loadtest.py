...
* EMERGENCY STOP. A flag in Redis halts the run within one tick, and — because
  the flag lives in Redis, not memory — the stop button works even when it is
  pressed on a different pod than the one running the generator.
...
def _clamp(rate: int, concurrency: int, duration: int) -> tuple[int, int, int]:
...
class LoadTester:
    ...
    async def start(self, rate: int, concurrency: int, duration: int, requested_by: str) -> dict:
        ...
        got = await state.r().set(
            K_LOCK, POD_NAME, nx=True, ex=duration + 15
        )
        if not got:
    ...
    async def request_stop(self, who: str) -> dict:
    ...
    async def emergency_stop(self, who: str) -> dict:
    ...
    async def _should_stop(self, deadline: float, sent: int, errors: int) -> str | None:
    ...
    async def _run(self, rate: int, concurrency: int, deadline: float):
        interval = 1.0 / rate
