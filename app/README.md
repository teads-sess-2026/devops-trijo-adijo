...
```bash
# 1. Redis
sudo service redis-server start     # or: docker run -d -p 6379:6379 redis:7-alpine

# 2. Deps in a venv
python3 -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt

# 3. Run (from inside app/)
REDIS_URL=redis://localhost:6379/0 ADMIN_TOKEN=dev uvicorn main:app --reload
```
